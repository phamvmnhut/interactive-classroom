...
var SpiffFormEntryField = function() {
    ...
    this.update = function() {
        if (!that._div)
            return;
        that._div.empty();
        that._div.append('<input type="text"/>');
        that._div.append('<span class="spiffform-item-error"></span>');
    ...
    this.update_properties = function(elem) {
        ...
        // Default value.
        var that = this;
        elem.append('<label >'+ 'Content' + ': <input type="text" name="default"/></label>');
...
var SpiffFormTextArea = function() {
    ...
    this.update = function() {
        if (!that._div)
            return;
        that._div.empty();
        that._div.append('<textarea></textarea>');
        that._div.append('<span class="spiffform-item-error"></span>');
    ...
    this.update_properties = function(elem) {
        ...
        // Default value.
        var that = this;
        elem.append('<div><label>Content: <textarea></texarea></label></div>');
...
// Choise
// -----------------------
var SpiffFormChoise = function() {
    this._label = 'Please send more spam to my inbox';
    this._name = 'Choise';
    this._question = "Enter Sentence"
    this._valueA = "Answer A";
    this._valueB = "Answer B";
    this._value = "A";
    var that = this;
    ...
    this.update = function() {
        if (!that._div)
            return;
        that._div.empty();
        that._div.append('<label>' + that._question + '</label>');
        that._div.append('<label><input type="checkbox" name="valueA"/>' + that._valueA + '</label>');
        that._div.append('<label><input type="checkbox" name="valueB"/>' + that._valueB + '</label>');
        that._div.append('<span class="spiffform-item-error"></span>');
        var checked = that._value === "A" ? true : false;
        var checkboxA = that._div.find('input[name=valueA]');
        checkboxA.prop('checked', checked);
        ...
        var checkboxB = that._div.find('input[name=valueB]');
        checkboxB.prop('checked', !checked);
    ...
    this.update_properties = function(elem) {
        ...
        var that = this;
        elem.append('<label >'+ 'Question' + ': <input type="text" name="question"/></label>');
        elem.append('<br/>');
        elem.append('<label >'+ 'AnswerA' + ': <input type="text" name="valueA"/></label>');
        elem.append('<br/>');
        elem.append('<label >'+ 'AnswerB' + ': <input type="text" name="valueB"/></label>');
        elem.append('<br/>');

        var question = elem.find('input[name=question]');
        question.val(this._question);
        question.bind('keyup mouseup change', function() {
            that._question = $(this).val();
            that.update();
        });

        var inputA = elem.find('input[name=valueA]');
        inputA.val(this._valueA);
        inputA.bind('keyup mouseup change', function() {
            that._valueA = $(this).val();
            that.update();
        });
        
        var inputB = elem.find('input[name=valueB]');
        inputB.val(this._valueB);
        inputB.bind('keyup mouseup change', function() {
            that._valueB = $(this).val();
            that.update();
        });
...
SpiffFormChoise.prototype = new SpiffFormElement();
spiffform_elements.choise = SpiffFormChoise;
// -----------------------
// Date Picker
// -----------------------
...
var SpiffFormRadioList = function() {
    this._name = 'Radio List';
    this._label = 'Select one';
    this._items = [];
    var that = this;
